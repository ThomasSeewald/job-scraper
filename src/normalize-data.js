...
class DataNormalizer {
    ...
    /**
     * Normalize all data from job_scrp_arbeitsagentur_jobs_v2 into job_scrp_employers and job_positions
     */
    async normalizeAllData(batchSize = 2000) {
        ...
        try {
            // Get total count
            const countResult = await client.query('SELECT COUNT(*) as total FROM job_scrp_arbeitsagentur_jobs_v2');
        ...
        const updateStatsQuery = `
            ...
            SET 
                total_job_positions = (
                    ...
                    WHERE employer_id = job_scrp_employers.id AND job_type = 'job'
                ),
                total_ausbildung_positions = (
                    SELECT COUNT(*) FROM job_positions 
                    WHERE employer_id = job_scrp_employers.id AND job_type = 'ausbildung'
                ),
                unique_beruf_count = (
                    SELECT COUNT(DISTINCT beruf) FROM job_positions 
                    WHERE employer_id = job_scrp_employers.id
                )
        `;
