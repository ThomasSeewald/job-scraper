...
async function scrapeNewestEmployers(maxEmployers = 20) {
    console.log('🏢 Starting newest job_scrp_employers scraper (with optimization)...');
    console.log(`🎯 Target: ${maxEmployers} job_scrp_employers from newest jobs`);
    ...
    try {
        ...
        // Get newest job_scrp_employers that need email extraction
        const query = `
            SELECT 
                a.arbeitgeber,
                COUNT(*) as job_count,
            ...
            ORDER BY MAX(a.id) DESC  -- Newest job_scrp_employers first
            LIMIT $1
        `;
        
        const result = await client.query(query, [maxEmployers]);
        const job_scrp_employers = result.rows;
        
        console.log(`📋 Found ${job_scrp_employers.length} job_scrp_employers with unscraped jobs`);
        ...
        for (let i = 0; i < job_scrp_employers.length; i++) {
            ...
            const progress = `[${i + 1}/${job_scrp_employers.length}]`;
