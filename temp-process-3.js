...
async function processSingleEmployer() {
    console.log('🚀 Process 3 starting for employer: timecon GmbH   Co. KG');
    console.log('🔗 URL: https://www.arbeitsagentur.de/jobsuche/jobdetail/14407-42102-S');
    ...
    try {
        ...
        await page.goto('https://www.arbeitsagentur.de/jobsuche/jobdetail/14407-42102-S', {
        ...
        if (captchaImage) {
            console.log('🧩 Process 3: CAPTCHA detected!');
        ...
        const emailResult = emailExtractor.extractEmails(pageContent, 'Buchhalter (m/w/d)', 'timecon GmbH   Co. KG');
        
        console.log('\n📊 Process 3 Results:');
        console.log('   Employer: timecon GmbH   Co. KG');
        ...
        console.log('\n✅ Process 3 completed - browser remains open for inspection');
        console.log('🔍 You can now inspect the page and close the browser when ready');
        
    } catch (error) {
        console.error('❌ Process 3 error:', error.message);
    }
    
    // Keep the process alive - DO NOT close browser
    console.log('⏸️ Process 3 waiting... (Press Ctrl+C to exit)');
