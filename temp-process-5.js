...
async function processSingleEmployer() {
    console.log('🚀 Process 5 starting for employer: Universitätsklinikum C.G.Carus an der TU Dresden');
    console.log('🔗 URL: https://www.arbeitsagentur.de/jobsuche/jobdetail/18237-43435886-65-S');
    ...
    try {
        ...
        await page.goto('https://www.arbeitsagentur.de/jobsuche/jobdetail/18237-43435886-65-S', {
        ...
        if (captchaImage) {
            console.log('🧩 Process 5: CAPTCHA detected!');
        ...
        const emailResult = emailExtractor.extractEmails(pageContent, 'Oberarzt/-ärztin (m/w/d) Universitätszentrum f. Orthopädie, Unfall- u. Plastische Chirurgie', 'Universitätsklinikum C.G.Carus an der TU Dresden');
        
        console.log('\n📊 Process 5 Results:');
        console.log('   Employer: Universitätsklinikum C.G.Carus an der TU Dresden');
        ...
        console.log('\n✅ Process 5 completed - browser remains open for inspection');
        console.log('🔍 You can now inspect the page and close the browser when ready');
        
    } catch (error) {
        console.error('❌ Process 5 error:', error.message);
    }
    
    // Keep the process alive - DO NOT close browser
    console.log('⏸️ Process 5 waiting... (Press Ctrl+C to exit)');
